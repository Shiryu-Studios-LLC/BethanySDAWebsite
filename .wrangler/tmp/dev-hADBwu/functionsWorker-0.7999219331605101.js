...
// .wrangler/tmp/pages-Te5ZbC/functionsWorker-0.7999219331605101.mjs
...
__name2(isLocalDevelopment, "isLocalDevelopment");
...
__name2(verifyAuthentication, "verifyAuthentication");
async function onRequest(context22) {
  ...
  try {
    if (request.method === "GET" && !path) {
      return await listPages(request, env22, corsHeaders);
    }
    if (request.method === "GET" && path) {
      return await getPageBySlug(request, env22, corsHeaders, path);
    }
    if (request.method === "POST" && !path) {
      return await createPage(request, env22, corsHeaders);
    }
    if (request.method === "PUT" && path) {
      return await updatePage(request, env22, corsHeaders, path);
    }
    if (request.method === "DELETE" && path) {
      return await deletePage(request, env22, corsHeaders, path);
    }
    return jsonResponse({ error: "Not found" }, 404, corsHeaders);
  } catch (error32) {
    console.error("API Error:", error32);
    return jsonResponse({ error: error32.message }, 500, corsHeaders);
  }
}
__name(onRequest, "onRequest");
__name2(onRequest, "onRequest");
async function listPages(request, env22, corsHeaders) {
...
__name2(listPages, "listPages");
async function getPageBySlug(request, env22, corsHeaders, slug) {
  const { results } = await env22.DB.prepare(`
...
__name2(getPageBySlug, "getPageBySlug");
async function createPage(request, env22, corsHeaders) {
  ...
  const { results: existing } = await env22.DB.prepare(
...
async function updatePage(request, env22, corsHeaders, id) {
  if (!verifyAuthentication(request)) {
    return jsonResponse({ error: "Unauthorized" }, 401, corsHeaders);
  }
  const page = await request.json();
...
__name2(updatePage, "updatePage");
async function deletePage(request, env22, corsHeaders, id) {
  if (!verifyAuthentication(request)) {
    return jsonResponse({ error: "Unauthorized" }, 401, corsHeaders);
  }
  await env22.DB.prepare("DELETE FROM pages WHERE id = ?").bind(id).run();
  return jsonResponse({ success: true, message: "Page deleted successfully" }, 200, corsHeaders);
}
__name(deletePage, "deletePage");
__name2(deletePage, "deletePage");
...
__name(isLocalDevelopment2, "isLocalDevelopment2");
__name2(isLocalDevelopment2, "isLocalDevelopment");
...
__name(verifyAuthentication2, "verifyAuthentication2");
__name2(verifyAuthentication2, "verifyAuthentication");
async function onRequest2(context22) {
  ...
  try {
    if (path === "site") {
      return handleSiteSettings(request, env22, corsHeaders);
    } else if (path === "homepage") {
      return handleHomepageSettings(request, env22, corsHeaders);
    } else if (path === "visit-page") {
      return handleVisitPageSettings(request, env22, corsHeaders);
    } else if (path === "about-page") {
      return handleAboutPageSettings(request, env22, corsHeaders);
    }
    return jsonResponse2({ error: "Not found" }, 404, corsHeaders);
  } catch (error32) {
    console.error("API Error:", error32);
    return jsonResponse2({ error: error32.message }, 500, corsHeaders);
  }
}
__name(onRequest2, "onRequest2");
__name2(onRequest2, "onRequest");
async function handleSiteSettings(request, env22, corsHeaders) {
  if (request.method === "GET") {
    const { results } = await env22.DB.prepare("SELECT key, value FROM site_settings").all();
    const settings = {};
    results.forEach((row) => {
  ...
  if (request.method === "PUT") {
    ...
    const stmt = env22.DB.prepare("INSERT OR REPLACE INTO site_settings (key, value, updated_at) VALUES (?, ?, CURRENT_TIMESTAMP)");
...
__name2(handleSiteSettings, "handleSiteSettings");
async function handleHomepageSettings(request, env22, corsHeaders) {
  if (request.method === "GET") {
    const { results } = await env22.DB.prepare("SELECT * FROM homepage_settings WHERE id = 1").all();
...
__name2(handleHomepageSettings, "handleHomepageSettings");
async function handleVisitPageSettings(request, env22, corsHeaders) {
  if (request.method === "GET") {
    const { results } = await env22.DB.prepare("SELECT * FROM visit_page_settings WHERE id = 1").all();
    const settings = results[0] || {};
...
__name2(handleVisitPageSettings, "handleVisitPageSettings");
async function handleAboutPageSettings(request, env22, corsHeaders) {
  if (request.method === "GET") {
    const { results } = await env22.DB.prepare("SELECT * FROM about_page_settings WHERE id = 1").all();
    const settings = results[0] || {};
...
__name2(handleAboutPageSettings, "handleAboutPageSettings");
...
__name(isLocalDevelopment3, "isLocalDevelopment3");
__name2(isLocalDevelopment3, "isLocalDevelopment");
...
__name(verifyAuthentication3, "verifyAuthentication3");
__name2(verifyAuthentication3, "verifyAuthentication");
async function onRequestGet({ request, env: env22 }) {
  try {
    ...
    if (!env22.MEDIA_BUCKET) {
      return new Response(JSON.stringify({ error: "R2 bucket not configured" }), {
        status: 500,
        headers: { "Content-Type": "application/json" }
      });
    }
    const listed = await env22.MEDIA_BUCKET.list();
    const files = listed.objects.map((obj) => ({
      fileKey: obj.key,
      fileName: obj.key.split("/").pop(),
      publicUrl: `${env22.R2_PUBLIC_URL}/${obj.key}`,
      size: obj.size,
    ...
    console.error("List error:", error32);
    return new Response(JSON.stringify({ error: error32.message }), {
      status: 500,
      headers: { "Content-Type": "application/json" }
    });
  }
...
async function onRequestDelete({ request, env: env22 }) {
  try {
    if (!verifyAuthentication3(request)) {
      return new Response(JSON.stringify({ error: "Unauthorized" }), {
        status: 401,
    ...
    await env22.MEDIA_BUCKET.delete(fileKey);
    return new Response(JSON.stringify({ success: true }), {
      status: 200,
      headers: { "Content-Type": "application/json" }
    });
  } catch (error32) {
    console.error("Delete error:", error32);
    return new Response(JSON.stringify({ error: error32.message }), {
      status: 500,
      headers: { "Content-Type": "application/json" }
    });
...
__name(isLocalDevelopment4, "isLocalDevelopment4");
__name2(isLocalDevelopment4, "isLocalDevelopment");
...
__name(verifyAuthentication4, "verifyAuthentication4");
__name2(verifyAuthentication4, "verifyAuthentication");
async function onRequestPost({ request, env: env22 }) {
  try {
    ...
    await env22.MEDIA_BUCKET.put(fileKey, file.stream(), {
      httpMetadata: {
        contentType: file.type
      }
    });
    const publicUrl = `${env22.R2_PUBLIC_URL}/${fileKey}`;
    ...
    console.error("Upload error:", error32);
    return new Response(JSON.stringify({ error: error32.message }), {
      status: 500,
      headers: { "Content-Type": "application/json" }
    });
...
__name(isLocalDevelopment5, "isLocalDevelopment5");
__name2(isLocalDevelopment5, "isLocalDevelopment");
...
__name(verifyAuthentication5, "verifyAuthentication5");
__name2(verifyAuthentication5, "verifyAuthentication");
async function onRequest3(context22) {
  ...
  try {
    ...
    console.error("Reset error:", error32);
    return new Response(JSON.stringify({
...
// .wrangler/tmp/bundle-ULsOrj/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-ULsOrj/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.7999219331605101.js.map
